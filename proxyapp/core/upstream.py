...
def connect_via(route, host: str, port: int, timeout: float = 12.0) -> socket.socket:
    ...
    connect = connect_socks5 if up.type == "socks5" else connect_http
    # 上游（如 FLClash）切换节点/短暂重启时会瞬时失败：小退避重试，避免应用"失效"
    last = None
    for attempt in range(3):
        try:
            return connect(up, host, port, timeout)
    ...
    if last is not None:
        raise last
    raise UpstreamError(f"无法连接上游代理: {host}:{port}")
